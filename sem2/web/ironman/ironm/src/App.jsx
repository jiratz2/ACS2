import { useState } from 'react'
import './App.css'


function App() {
  const [content, setContent] = useState([
    {id: 1, title: "What's a statue?", body: "Lorem ipsum dolor sit amet consectetur, adipisicing elit. Velit, veniam."},
    {id: 2, title: "How to display?", body: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Ab, fugiat?"},
    {id: 3, title: "Meet collectors", body: "Lorem ipsum dolor sit, amet consectetur adipisicing elit. Vitae, deleniti?" },
    {id: 4, title: "Start your journey", body: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Iste, earum."}
  ]);

  return (
    <>
      <div>
        <h4>Happy collecting</h4>
        <h3>TSC Channel</h3>
        <h2>The source for your statue journey</h2>
        <p>Lorem ipsum dolor sit, amet consectetur adipisicing elit. Voluptate quisquam eum officiis ducimus nesciunt repudiandae suscipit? Amet maiores id voluptatem rem perferendis. Natus nemo architecto hic aliquid animi officiis quod?</p>
        <input type="text" placeholder='E-mail address' />
        <button >Join US!</button>
        
        {content.map((content) => (
          <div key={content.id}>
            <img src="data:image/svg+xml;base64," alt="icon" />
            <h2>{content.title}</h2>
            <p>{content.body}</p>
          </div>
        ))}
      </div>
    </>
  );
}

export default App;
